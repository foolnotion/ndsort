...
auto sort_impl_m(detail::flat_fitness const& ff, double eps) -> fronts
{
    ...
    auto compare = [&](std::size_t a, std::size_t b) -> dominance {
        ...
        for (std::size_t k = 0; k < m; ++k) {
            ...
            rhs |= static_cast<uint8_t>(vb < va && (va - vb) > eps);
        }
    ...
    while (sorted_count < n) {
        ...
        for (std::size_t i = 0; i < n; ++i) {
            if (dominated_or_sorted(i)) { continue; }
            for (std::size_t j = i + 1; j < n; ++j) {
                if (dominated_or_sorted(j)) { continue; }
                auto const dom = compare(i, j);
                if (dom == dominance::right) { set_bit(dominated, i); }
                if (dom == dominance::left)  { set_bit(dominated, j); }
                if (get_bit(dominated, i))   { break; }
            }
