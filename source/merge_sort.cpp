...
public:
    ...
    auto update_solution_dominance(std::size_t id) -> bool
    {
        ...
        while (fw <= lw && (m_bitsets[id][fw] & m_incremental[fw]) == 0) { ++fw; }
        while (fw <= lw && (m_bitsets[id][lw] & m_incremental[lw]) == 0) { --lw; }
    ...
    void compute_solution_ranking(std::size_t id, std::size_t init_id)
    {
        ...
        for (auto w = fw; w <= lw; ++w) {
            ...
            do {
                ...
                if (r >= rank) { rank = r + 1; }
                ++i;
                if (i < k_word_bits) { i += static_cast<std::size_t>(std::countr_zero(word >> i)); }
            } while (i < k_word_bits && rank <= m_word_ranking[w]);
            if (rank > m_max_rank) { m_max_rank = rank; break; }
        }
        m_ranking[id]      = rank;
        m_ranking0[init_id] = rank;
        auto wi = id / k_word_bits;
        if (rank > m_word_ranking[wi]) { m_word_ranking[wi] = rank; }
    }
    ...
    auto init_solution_bitset(std::size_t id) -> bool
    {
        ...
        if (m_inc_last >= wi) {
            ...
            if (m_bitsets[id][lw] == 0) { --m_ranges[id][k_last]; }
        }
        return true;
    }
...
// eps has no effect: dominance is implicit in the objective-wise sort order, with no
// tolerance applied to individual comparisons. For epsilon-box dominance use a sorter
