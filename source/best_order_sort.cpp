...
template <typename T>
auto best_order_sorter::sort_impl(detail::flat_fitness<T> const& ff, double eps) const -> fronts
{
    ...
    if (m > 1) {
        ...
        for (int i = 0; i < n; ++i) {
            items[i] = { i, ff.at(1, static_cast<std::size_t>(i)) };
        }
        detail::radix_sort(items.data(), scratch.data(), ni);
        sorted_by_objective[1].resize(n);
        for (int i = 0; i < n; ++i) {
            sorted_by_objective[1][i] = items[i].index;
        }

        for (auto j = 2; j < m; ++j) {
            for (auto& [idx, v] : items) {
                v = ff.at(static_cast<std::size_t>(j), static_cast<std::size_t>(idx));
            }
            detail::radix_sort(items.data(), scratch.data(), ni);
            sorted_by_objective[j].resize(n);
            for (int i = 0; i < n; ++i) {
                sorted_by_objective[j][i] = items[i].index;
            }
        }
    }
    ...
    auto add_to_rank_set = [&](int s, int j) {
        ...
        if (r >= static_cast<int>(ss.size())) {
            ss.resize(static_cast<std::size_t>(r) + 1UZ);
        }
        ss[r].push_back(s);
    };
    ...
    auto dominated_by = [&](int s, int t) -> bool {
        return std::ranges::none_of(comparison_sets[t], [&](int k) {
            ...
            return ff.at(ks, static_cast<std::size_t>(s)) < ff.at(ks, static_cast<std::size_t>(t)) - static_cast<T>(eps);
        });
    };
    ...
    auto find_rank = [&](int s, int j) {
        ...
        for (auto k = 0; k < rank_count; ++k) {
            ...
            for (auto t : solution_sets[j][k]) {
                if (dominated = dominated_by(s, t); dominated) {
                    break;
                }
            }
            if (!dominated) {
